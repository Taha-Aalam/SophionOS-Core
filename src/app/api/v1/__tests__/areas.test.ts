...
// --- Mocks -----------------------------------------------------------------
vi.mock("@/lib/services/area.service", () => ({
  areaService: {
    list: vi.fn(),
    getGroupedByType: vi.fn(),
    create: vi.fn(),
...
import { GET, POST } from "../areas/route";

const mockRequireAuth = vi.mocked(requireAuth);
const mockList = vi.mocked(areaService.list);
const mockCreate = vi.mocked(areaService.create);
...
describe("GET /api/v1/areas", () => {
  it("returns a paginated list (200)", async () => {
    mockList.mockResolvedValue([
      { id: "a1", name: "Health", type: "Personal" },
    ] as never);

    const res = await GET(new NextRequest("http://localhost/api/v1/areas"));
...
describe("POST /api/v1/areas", () => {
  it("creates an area (201)", async () => {
    mockCreate.mockResolvedValue({ id: "a2", name: "Career", type: "Personal" } as never);

    const res = await POST(
      jsonRequest("http://localhost/api/v1/areas", "POST", { name: "Career" }),
    );
    expect(res.status).toBe(201);

    const json = await res.json();
    expect(json.data).toMatchObject({ id: "a2", name: "Career" });
  });

  it("returns 415 when content-type is not application/json", async () => {
    const res = await POST(
      new NextRequest("http://localhost/api/v1/areas", {
        method: "POST",
