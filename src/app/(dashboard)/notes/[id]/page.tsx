...
export default async function NoteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const note = await serverFetchNoteByIdentifier(supabase, userId, id);
  await queryClient.prefetchQuery({
    queryKey: [NOTES_QUERY_KEY, "detail", userId, id],
    queryFn: () => Promise.resolve(note),
  });

  await queryClient.prefetchQuery({
    queryKey: [NOTE_TYPES_QUERY_KEY, userId],
    queryFn: () => serverFetchNoteTypes(supabase, userId),
  });
