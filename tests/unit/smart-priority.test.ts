...
describe('calculateSmartPriority', () => {
  ...
  describe('bounds', () => {
    it('minimum score is 1 (low priority, no date, no goals, no flags)', () => {
      expect(
        calculateSmartPriority({ priority: PRIORITY.LOW, dueDate: null, goalCount: 0, isImportant: false, isUrgent: false })
      ).toBe(1);
    ...
    it('maximum score is 5 (all factors at max)', () => {
      expect(
        calculateSmartPriority({ priority: PRIORITY.HIGH, dueDate: '2020-01-01', goalCount: 10, isImportant: true, isUrgent: true })
      ).toBe(5);
    ...
    it('never returns below 1 regardless of inputs', () => {
      expect(
        calculateSmartPriority({ priority: PRIORITY.LOW, dueDate: future(999), goalCount: 0, isImportant: false, isUrgent: false })
  ...
  describe('due date weight', () => {
    it('overdue scores higher than no-date when other factors constant', () => {
      ...
      const base = { priority: PRIORITY.MEDIUM, goalCount: 0, isImportant: true, isUrgent: true };
    ...
    it('overdue scores higher than far-future due date', () => {
      const base = { priority: PRIORITY.HIGH, goalCount: 0, isImportant: false, isUrgent: false };
    ...
    it('due in 14+ days has near-zero due weight (≤ no-date score)', () => {
      const base = { priority: PRIORITY.LOW, goalCount: 0, isImportant: false, isUrgent: false };
  ...
  describe('goal alignment weight', () => {
    it('more goals = higher or equal score up to the cap', () => {
      const base = { priority: PRIORITY.LOW, dueDate: null, isImportant: false, isUrgent: false };
  ...
  describe('Eisenhower weight', () => {
    ...
    const base = { priority: PRIORITY.LOW, dueDate: null, goalCount: 1 };
    ...
    it('important-only raw weight (0.7) is >= urgent-only (0.5)', () => {
      ...
      const b = { priority: PRIORITY.HIGH, dueDate: '2020-01-01', goalCount: 0 };
  ...
  describe('combined factors', () => {
    it('high-priority overdue task with 2 goals and both flags scores 5', () => {
      expect(
        calculateSmartPriority({ priority: PRIORITY.HIGH, dueDate: '2020-01-01', goalCount: 2, isImportant: true, isUrgent: true })
      ).toBe(5);
    ...
    it('low-priority far-future task with no goals scores 1', () => {
      expect(
        calculateSmartPriority({ priority: PRIORITY.LOW, dueDate: future(30), goalCount: 0, isImportant: false, isUrgent: false })
      ).toBe(1);
